...
Or run specific phases of the end-to-end pipeline:
```bash
bash foundation/deploy_all.sh                    # all phases (default)
bash foundation/deploy_all.sh --phases infra     # terraform only (deploy agent later)
bash foundation/deploy_all.sh --phases agent     # redeploy agent only (infra done)
bash foundation/deploy_all.sh --phases sgp       # recreate SGP policy only
bash foundation/deploy_all.sh --phases test      # rerun guardrail tests only
bash foundation/deploy_all.sh --phases agent,sgp,test  # skip infra, run rest
```
...
## Enabling IAP — Centralized Identity Enforcement (Optional)

IAP adds an identity layer to the **ingress gateway** that controls who can
call ANY agent behind this gateway. It answers "is this caller allowed to use
this gateway?" before content policies (Model Armor, SGP) ever run.

> [!NOTE]
> **IAP at the gateway is NOT the same as standard Cloud IAP on web apps.**
> No OAuth brand, OAuth client, or browser flow is needed.
> Official docs pattern: `REQUEST_AUTHZ` authz extension → `iap.googleapis.com`
> (global endpoint), with mandatory `iapPolicyVersion = "V1"` metadata.

### Why use IAP vs. Vertex AI IAM?

| | Vertex AI IAM | IAP at Gateway |
|---|---|---|
| Scope | Per Reasoning Engine | ALL agents behind this gateway |
...
> gateway. Agents deployed via `deploy_chat_agent.sh` are auto-registered.
> No extra steps needed with the standard deploy flow.

### Start in Dry-Run Mode (Audit First)

Test without enforcing by setting `iamEnforcementMode = "DRY_RUN"` in the
extension metadata. This logs IAP decisions but does not block traffic:

```hcl
# In 03_security_and_gateways.tf — temporarily during testing only
metadata = {
  iapPolicyVersion   = "V1"
  iamEnforcementMode = "DRY_RUN"  # remove when ready to enforce
}
```
