...
# In mod-agw-foundation, the default agent lives at:
AGENT_DIR="agents/chat-agent"          # adjust if using --agent-path

cat "$AGENT_DIR/agent.py"
ls "$AGENT_DIR/"
cat "$AGENT_DIR/requirements.txt" 2>/dev/null || cat "$AGENT_DIR/pyproject.toml"
cat "$AGENT_DIR/.env"
```
...
- [ ] Is `GOOGLE_API_USE_MTLS_ENDPOINT=never` in `$AGENT_DIR/.env`? (OTEL crash fix)
- [ ] Is there already a `lib/gateway_agent/` directory at the foundation root?
