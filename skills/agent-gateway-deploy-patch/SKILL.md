...
> [!IMPORTANT]
> **Scope: This skill documents the Agent Gateway deployment pattern for org-policy-enforced GCP environments.**
> If you are using `mod-agw-foundation`, you do NOT need to apply this skill manually.
> `deploy_chat_agent.sh` runs `scripts/patch_sdk_for_rest_create.py` automatically as
> part of every deploy — the patch, the `.pth` file, and the `contextSpec` strip are
> all handled for you. Only read the deep-dive sections below if you are debugging
> a patch failure.
...
>    In org-policy-enforced GCP environments, injecting `agentGatewayConfig` into `CreateReasoningEngine` or `UpdateReasoningEngine` can cause the container to crash on startup with error code 13 (`INTERNAL`). This may occur on **BOTH CREATE and PATCH** operations. The root cause is typically the gateway infrastructure (PSC network attachment, SWP) needing reconfiguration after org migration.
>
...
>    - **Step 2**: Optionally PATCH to add gateway config later (may crash until gateway infrastructure is reconfigured).
>
> 3. **Org Policy does NOT always enforce agentGatewayConfig**:
>    In some project configurations, Reasoning Engines can be created successfully without `agentGatewayConfig`. The org policy constraint may have been relaxed or removed during org migration.
>
> 4. **Project Number vs. Project Name**:
>    Gateway resources in the `networkservices` API are keyed by **project NUMBER** (`YOUR_PROJECT_NUMBER`), not project NAME (`YOUR_PROJECT_ID`). `versions.env` was updated to use project number for gateway URIs.
>
...
>
> 6. **Dependency pinning fix**:
>    Pinning `google-adk` to a specific version may cause `code 3` (build failure). Use `google-adk>=1.5.0` instead of hard pinning.
>
...
> ```bash
> # Both are valid:
> AGENT_GATEWAY_INGRESS=projects/YOUR_PROJECT_NUMBER/locations/...
> AGENT_GATEWAY_INGRESS=projects/YOUR_PROJECT_ID/locations/...
> ```
> The Network Services API lists gateways with project ID, but some configurations
> use project NUMBER in `agentGatewayConfig`. Either format is accepted.
> **The gateway binding failure post-org-migration is NOT caused by the path format**
> — it affects both formats equally.
