...
# Note: use ${PREFIX:+${PREFIX}-} to handle empty prefix correctly
POLICY_ID="${PREFIX:+${PREFIX}-}${SAFE_NAME}-sgp"
...
> [!IMPORTANT]
> Do NOT use `bash scripts/create_sgp_policy.sh` for additional agents deployed
> with `--agent-name`. That script always reads `agent_name` from `terraform.tfvars`
> and will create a policy for the PRIMARY agent, not the additional one.
> Always use the **manual fallback (Step 4)** for any agent beyond the first.
